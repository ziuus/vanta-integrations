...
pub fn metadata() -> FnResult<Vec<u8>> {
    Ok(vanta_ext_sdk::ExtensionMetadata::new(
        "mediadeck_players",
        "MediaDeck Players Component",
        "0.1.0",
        "Players micro-extension.",
        API_VERSION_TELEMETRY,
    )
    .to_json())
}
...
pub fn widgets(_: ()) -> FnResult<Vec<u8>> {
    let ids = serde_json::json!(["media_players"]);
    Ok(serde_json::to_vec(&ids).unwrap_or_default())
}

#[plugin_fn]
pub fn render_widget(id: String) -> FnResult<Vec<u8>> {
    if id != "media_players" { return Ok(unavailable("?", "?").to_json()); }
    let Ok(state) = fetch_data() else { return Ok(unavailable("MEDIA", "No telemetry").to_json()); };
    let mut lines = vec![];
    for t in state.all_tracks {
        let prefix = if t.is_active { "▶" } else { " " };
        lines.push(Line::new(vec![Span { content: format!("{} {}", prefix, t.player_name), style: if t.is_active { Some(Style::fg(Color::CYAN)) } else { None } }]));
    }
    let w = Widget::paragraph(lines).block(Block::titled(" PLAYERS "));
    Ok(w.to_json())
}
