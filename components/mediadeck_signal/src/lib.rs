...
pub fn render_widget(id: String) -> FnResult<Vec<u8>> {
    ...
    if let Some(t) = get_active(&state) {
        let w = Widget::paragraph(vec![
            Line::new(vec![Span { content: format!("{} / {}", format_dur(t.position_ms), format_dur(t.length_ms)), style: None }]),
        ]).block(Block::titled(" SIGNAL "));
        Ok(w.to_json())
    } else {
