...
#[plugin_fn]
pub fn metadata(_: ()) -> FnResult<Vec<u8>> {
    Ok(ExtensionMetadata::new(
        ID,
        "IOWatch",
        VERSION,
        "Per-process disk I/O throughput with temporal activity history",
...
pub fn render_widget(widget_id: String) -> FnResult<Vec<u8>> {
    let widget = match widget_id.as_str() {
        ...
        "io_top" => build_io_top(40, 10),
        "io_activity" => build_io_activity(60, 10),
        other => error_widget("IOWATCH", &format!("unknown widget: {other}")),
    };
    Ok(widget.to_json())
}
...
mod tests {
    use super::*;
    use vanta_ext_sdk::telemetry::{IoProcess, IoSnapshot};

    pub fn make_snap(procs: Vec<(&str, f64, f64)>) -> IoSnapshot {
        let processes: Vec<IoProcess> = procs
            .into_iter()
            .enumerate()
            .map(|(i, (name, r, w))| IoProcess {
                pid: 1000 + i as u32,
                name: name.to_string(),
                read_bps: r,
                write_bps: w,
            ...
            .collect();
        let total_read: f64 = processes.iter().map(|p| p.read_bps).sum();
        let total_write: f64 = processes.iter().map(|p| p.write_bps).sum();
        IoSnapshot {
    ...
    #[test]
    fn fmt_rate_unit_selection() {
        assert!(fmt_rate(500.0).contains("B/s"));
        assert!(fmt_rate(2048.0).contains("K/s"));
        assert!(fmt_rate(2.0 * 1024.0 * 1024.0).contains("M/s"));
        assert!(fmt_rate(2.0 * 1024.0 * 1024.0 * 1024.0).contains("G/s"));
    }

    #[test]
    fn fmt_rate_max_10_chars() {
        for bps in [0.0, 999.0, 1024.0, 1_048_576.0, 1_073_741_824.0] {
            let s = fmt_rate(bps);
            assert!(
                s.len() <= 10,
                "fmt_rate({bps}) = '{s}' is {} chars",
                s.len()
            );
        }
    }
    ...
    #[test]
    fn activity_bar_full() {
        let b = activity_bar(100.0, 100.0, 10);
        assert_eq!(b.chars().count(), 10);
        assert!(!b.contains('░'), "full bar has empty: {b}");
    }

    #[test]
    fn activity_bar_empty() {
        let b = activity_bar(0.0, 100.0, 10);
        assert_eq!(b.chars().count(), 10);
        assert!(!b.contains('█'), "empty bar has fill: {b}");
    }

    #[test]
    fn activity_bar_zero_max_no_panic() {
        let b = activity_bar(50.0, 0.0, 10);
        assert_eq!(b.chars().count(), 10); // all ░, no panic
    }
    ...
    fn rate_color_thresholds() {
        assert_eq!(rate_color(0.0), Color::WHITE);
        assert_eq!(rate_color(2.0 * 1024.0 * 1024.0), Color::GREEN);
        assert_eq!(rate_color(15.0 * 1024.0 * 1024.0), Color::YELLOW);
        assert_eq!(rate_color(60.0 * 1024.0 * 1024.0), Color::RED);
    }

    // ── ProcHistory ──────────────────────────────────────────────────────────

    #[test]
    fn proc_history_push_and_peak() {
        let mut h = ProcHistory::new(1234, "rust-analyzer");
        h.push(10.0 * 1024.0 * 1024.0);
        h.push(20.0 * 1024.0 * 1024.0);
        h.push(5.0 * 1024.0 * 1024.0);
        assert_eq!(h.peak_bps, 20.0 * 1024.0 * 1024.0);
        assert_eq!(h.current(), 5.0 * 1024.0 * 1024.0);
        assert_eq!(h.samples.len(), 3);
    }

    #[test]
    fn proc_history_caps_at_depth() {
        let mut h = ProcHistory::new(1, "test");
        for i in 0..HISTORY_DEPTH + 10 {
            h.push(i as f64);
        }
        assert_eq!(h.samples.len(), HISTORY_DEPTH, "capped at HISTORY_DEPTH");
        assert_eq!(
            h.current(),
            (HISTORY_DEPTH + 9) as f64,
            "newest sample must be last"
        );
    }

    #[test]
    fn proc_history_empty_current_is_zero() {
        let h = ProcHistory::new(1, "none");
    ...
    #[test]
    fn build_iowatch_with_data_serialises() {
        inject(make_snap(vec![
            ("chrome", 7.0 * 1024.0 * 1024.0, 9.0 * 1024.0 * 1024.0),
            ("postgres", 0.8 * 1024.0 * 1024.0, 12.4 * 1024.0 * 1024.0),
        ]));
        let w = build_iowatch(80, 24);
        let json = serde_json::to_string(&w).expect("serialises");
        assert!(!json.is_empty());
    }

    #[test]
    fn build_iowatch_idle_serialises() {
        inject(make_snap(vec![]));
        let w = build_iowatch(80, 24);
        let json = serde_json::to_string(&w).expect("serialises");
        assert!(!json.is_empty());
    ...
    #[test]
    fn build_io_activity_empty_history_loading() {
        STORE.with(|s| {
            let mut st = s.borrow_mut();
            st.history.clear();
            st.last = None;
        ...
        let w = build_io_activity(60, 10);
        let json = serde_json::to_string(&w).expect("serialises");
        assert!(!json.is_empty());
    }

    #[test]
    fn build_io_activity_with_history_serialises() {
        STORE.with(|s| {
            let mut st = s.borrow_mut();
            st.history.clear();
            let mut h = ProcHistory::new(100, "test-proc");
            for i in 0..20 {
                h.push(i as f64 * 1024.0 * 1024.0);
            }
            st.history.push(h);
            st.caps_done = true;
        });
    ...
    #[test]
    fn error_widget_serialises() {
        let w = error_widget("TEST", "something broke");
        let json = serde_json::to_string(&w).expect("serialises");
        assert!(!json.is_empty());
    }
}
