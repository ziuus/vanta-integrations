...
pub fn render_widget(widget_id: String) -> FnResult<Vec<u8>> {
    let widget = match widget_id.as_str() {
        ...
        "netscope_table" => build_netscope(80, 16, true, false, false),
        "netscope_activity" => build_netscope(50, 10, false, false, true),
        "netscope_summary" => build_netscope(40, 10, false, true, false),
        other => error_widget("NETSCOPE", &format!("unknown widget: {other}")),
    };
    Ok(widget.to_json())
}
...
mod tests {
    ...
    #[test]
    fn format_state_shortens_correctly() {
        assert_eq!(format_state("ESTABLISHED"), "ESTAB");
        assert_eq!(format_state("TIME_WAIT"), "TIME_W");
        assert_eq!(format_state("UNKNOWN"), "UNKNOWN");
    }

    #[test]
    fn format_age_works() {
        assert_eq!(format_age(2000), "2s");
        assert_eq!(format_age(65000), "1m");
    }

    #[test]
    fn is_listen_works() {
        assert!(is_listen("0.0.0.0:80"));
        assert!(is_listen("[::]:443"));
        assert!(!is_listen("127.0.0.1:80"));
    }
}
