...
pub fn metadata(_: ()) -> FnResult<Vec<u8>> {
    Ok(ExtensionMetadata::new(
        ...
        "PortWatch",
        VERSION,
        "Listening ports with process attribution and open/close activity",
...
pub fn render_widget(widget_id: String) -> FnResult<Vec<u8>> {
    let widget = match widget_id.as_str() {
        ...
        "port_listeners" => build_port_listeners(40, 12),
        "port_activity" => build_port_activity(50, 10),
        other => error_widget("PORTWATCH", &format!("unknown widget: {other}")),
    };
    Ok(widget.to_json())
}
...
mod tests {
    use super::*;
    use vanta_ext_sdk::telemetry::{ConnectionEntry, ConnectionsSnapshot};
    ...
    fn make_snap(entries: Vec<ConnectionEntry>) -> ConnectionsSnapshot {
        let established = entries.iter().filter(|c| c.state == "ESTABLISHED").count();
        let listen = entries.iter().filter(|c| c.state == "LISTEN").count();
        let time_wait = entries.iter().filter(|c| c.state == "TIME_WAIT").count();
        ConnectionsSnapshot {
            total: entries.len(),
            established,
            listen,
    ...
    #[test]
    fn local_port_invalid_returns_none() {
        let c = conn("tcp", "no-port-here", "0.0.0.0:0", "LISTEN", None, None);
        assert_eq!(c.local_port(), None);
    }
    ...
    fn extract_listeners_deduplicates_tcp_and_tcp6() {
        ...
        assert_eq!(listeners[0].process_name, "sshd");
    }
    ...
    fn extract_listeners_exposed_before_local() {
        ...
        assert_eq!(listeners[0].port, 22);
        assert!(listeners[0].exposed);
        // Local ports after.
        for l in &listeners[1..] {
            assert!(!l.exposed, "expected local: {:?}", l.port);
        }
    }
    ...
    #[test]
    fn port_color_ssh_is_red() {
        assert_eq!(port_color(22), Color::RED);
    }

    #[test]
    fn port_color_http_is_green() {
        assert_eq!(port_color(80), Color::GREEN);
        assert_eq!(port_color(443), Color::GREEN);
    }

    #[test]
    fn port_color_postgres_is_cyan() {
        assert_eq!(port_color(5432), Color::CYAN);
    }

    #[test]
    fn port_color_unknown_is_white() {
        assert_eq!(port_color(12345), Color::WHITE);
    }

    // ── PortEvent ─────────────────────────────────────────────────────────────

    #[test]
    fn port_event_age_str_seconds() {
        let ev = PortEvent {
            port: 3000,
            protocol: "tcp".into(),
        ...
        assert_eq!(ev.age_str(12_000), "7s ago");
    }

    #[test]
    fn port_event_age_str_minutes() {
        let ev = PortEvent {
            port: 3000,
            protocol: "tcp".into(),
            process_name: "node".into(),
        ...
        assert_eq!(ev.age_str(130_000), "2m ago");
    }

    // ── Widget builders ───────────────────────────────────────────────────────

    fn inject(snap: ConnectionsSnapshot) {
        let listeners = extract_listeners(&snap);
        STORE.with(|s| {
    ...
    #[test]
    fn build_portwatch_empty_serialises() {
        inject(make_snap(vec![]));
        let w = build_portwatch(80, 20);
        let json = serde_json::to_string(&w).expect("serialises");
        assert!(!json.is_empty());
    }
    ...
    #[test]
    fn build_port_activity_no_events_serialises() {
        STORE.with(|s| {
            let mut st = s.borrow_mut();
            st.activity.clear();
            st.caps_done = true;
        });
        let w = build_port_activity(50, 8);
        let json = serde_json::to_string(&w).expect("serialises");
        assert!(!json.is_empty());
    }

    #[test]
    fn build_port_activity_with_events_serialises() {
        STORE.with(|s| {
            let mut st = s.borrow_mut();
            st.activity.clear();
            st.activity.push(PortEvent {
                port: 3000,
                protocol: "tcp".into(),
                process_name: "node".into(),
                kind: EventKind::Opened,
                ts_ms: now_ms().saturating_sub(4_000),
            });
            st.activity.push(PortEvent {
                port: 8080,
                protocol: "tcp".into(),
                ...
                kind: EventKind::Closed,
                ts_ms: now_ms().saturating_sub(42_000),
            });
            st.caps_done = true;
        });
        let w = build_port_activity(50, 8);
    ...
    #[test]
    fn activity_ring_caps_at_limit() {
        STORE.with(|s| {
            let mut st = s.borrow_mut();
            st.activity.clear();
            ...
            assert_eq!(st.activity.len(), ACTIVITY_RING);
        });
    }
}
